...
# O Render inicia settings:app. O menu principal passa a mostrar Configurações.
core.LAYOUT = core.LAYOUT.replace(
    '<a href="/logout">↪ Sair</a>',
    '<a href="/configuracoes">⚙️ Configurações</a><a href="/logout">↪ Sair</a>'
)
...
def configurar_perfil(uid):
    ...
    with Session() as db:
        ...
        if session.get('role') == 'chefe' and (alvo.role == 'admin' or role == 'admin'):
            abort(403)
        anterior = alvo.role
        alvo.role = role
        log(db, 'ALTERAR_PERFIL', f'{alvo.username}: {anterior} -> {role}')
        db.commit()
...
def configurar_status(uid):
    with Session() as db:
        ...
        alvo.active = not alvo.active
        log(db, 'ALTERAR_STATUS_USUARIO', f'{alvo.username}: {"ativo" if alvo.active else "desativado"}')
        db.commit()
        flash('Status do usuário atualizado.')
    return redirect(url_for('configuracoes'))

TEMPLATE = '''<h1>Configurações</h1><div class="grid"><div class="form"><h2>🔐 Minha conta</h2><p class="muted">Sua senha nunca é exibida. Ela fica armazenada de forma protegida e só pode ser alterada mediante a senha atual.</p><p><b>Nome:</b> {{usuario.name}}<br><b>Login:</b> {{usuario.username}}<br><b>Perfil:</b> {{roles[usuario.role]}}</p><form method="post"><label>Senha atual<input type="password" name="current_password" autocomplete="current-password" required></label><label>Nova senha<input type="password" name="new_password" minlength="8" autocomplete="new-password" required></label><label>Confirmar nova senha<input type="password" name="confirm_password" minlength="8" autocomplete="new-password" required></label><button class="btn primary">Alterar senha</button></form></div><div class="panel"><h2>👤 O que cada usuário representa</h2>{% for role,info in role_info.items() %}<div class="role"><b>{{roles[role]}}</b><p class="muted">{{info}}</p></div>{% endfor %}</div></div>{% if manager %}<div class="panel" style="margin-top:18px"><h2>👥 Configurar usuários</h2><p class="muted">Defina o perfil e o status de cada conta. Nenhuma senha é mostrada nesta tela.</p><table><tr><th>Nome</th><th>Login</th><th>Perfil</th><th>Status</th><th>Ação</th></tr>{% for x in usuarios %}<tr><td>{{x.name}}</td><td>{{x.username}}</td><td><form method="post" action="/configuracoes/usuario/{{x.id}}/perfil"><select name="role" onchange="this.form.submit()">{% for r,n in roles.items() %}{% if not (myrole=='Chefe' and r=='admin') %}<option value="{{r}}" {% if x.role==r %}selected{% endif %}>{{n}}</option>{% endif %}{% endfor %}</select></form></td><td>{{'Ativo' if x.active else 'Desativado'}}</td><td>{% if x.id!=session['uid'] %}<form method="post" action="/configuracoes/usuario/{{x.id}}/status"><button class="btn">{{'Desativar' if x.active else 'Ativar'}}</button></form>{% endif %}</td></tr>{% endfor %}</table></div>{% endif %}'''
