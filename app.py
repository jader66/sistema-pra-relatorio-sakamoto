...
class Service(Base):
    ...
    id:Mapped[int]=mapped_column(primary_key=True); product_id:Mapped[int]=mapped_column(ForeignKey('products.id')); client:Mapped[str]=mapped_column(String(160),default=''); problem:Mapped[str]=mapped_column(Text,default=''); priority:Mapped[str]=mapped_column(String(30),default='Normal'); status:Mapped[str]=mapped_column(String(30),default='Pendente'); responsible_id:Mapped[int|None]=mapped_column(ForeignKey('users.id'),nullable=True); created_by:Mapped[int]=mapped_column(ForeignKey('users.id')); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow); started_at:Mapped[datetime|None]=mapped_column(DateTime,nullable=True); finished_at:Mapped[datetime|None]=mapped_column(DateTime,nullable=True); notes:Mapped[str]=mapped_column(Text,default='')
class Photo(Base):
...
def init_db():
    ...
    with engine.begin() as c:
        if engine.dialect.name=='sqlite':
            cols={r[1] for r in c.execute(text('PRAGMA table_info(products)')).fetchall()}
            if 'priority' not in cols: c.execute(text("ALTER TABLE products ADD COLUMN priority VARCHAR(30) DEFAULT 'Normal'"))
        else: c.execute(text("ALTER TABLE products ADD COLUMN IF NOT EXISTS priority VARCHAR(30) DEFAULT 'Normal'"))
...
def dashboard():
    with db() as d:
        ...
        services=q.all(); pending=sum(s.status=='Pendente' for s in services); active=sum(s.status=='Em andamento' for s in services); finished=sum(s.status=='Finalizado' for s in services); urgent=sum(priority_for(s)=='Urgente' for s in services); products=d.query(Product).count(); users=d.query(User).count()
