import os
import app as core
from app import app, Session, User, ROLES, ROLE_INFO, auth, manager, managers_only, log
from flask import request, redirect, url_for, render_template_string, flash, abort, session
from werkzeug.security import generate_password_hash, check_password_hash

# A nova camada de configuração não expõe credenciais no login.
# Como o Render inicia settings:app, o menu principal recebe Configurações.
core.LAYOUT=core.LAYOUT.replace('<a href="/logout">↪ Sair</a>','<a href="/configuracoes">⚙️ Configurações</a><a href="/logout">↪ Sair</a>')
...
def configuracoes():
    with Session() as db:
        ...
        if request.method=='POST':
            atual=request.form.get('current_password','');nova=request.form.get('new_password','');confirma=request.form.get('confirm_password','')
        ...
        body=render_template_string(TEMPLATE,usuario=u,usuarios=usuarios,roles=ROLES,role_info=ROLE_INFO,manager=manager(),myrole=ROLES.get(session.get('role'),''))
...
def configurar_perfil(uid):
    ...
    with Session() as db:
        ...
        if alvo.id==session['uid'] and session.get('role')=='admin' and role!='admin':
            flash('O Administrador não pode retirar o próprio perfil administrativo por esta tela.');return redirect(url_for('configuracoes'))
